...
function action(mode, type, selection) {
    ...
    // 第二步：根据选择的分类，展示二级物品列表
        ...
        if (config.isEquip) {
            if(selection == 2) {
                var selStr = "你想合成头盔吗？好！但你要小心。作为合成用的材料道具都会消失，你万一拿已经用卷轴#r加强#k过的道具来作为合成的材料以前的属性就会消失。要认真考虑啊#b";
                // 等于合成，需要提示，状态不添加：
                cm.sendNext(selStr);
            } else if(selection == 3) {
                var selStr = "你想合成盾牌吗？好！但你要小心。作为合成用的材料道具都会消失，你万一拿已经用卷轴#r加强#k过的道具来作为合成的材料以前的属性就会消失。要认真考虑啊#b";
                // 等于合成，需要提示，状态不添加：
                cm.sendNext(selStr);
            }
        } else {
