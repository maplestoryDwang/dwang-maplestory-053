...
public final class RemoteGachaponHandler extends AbstractPacketHandler {
    ...
    public final void handlePacket(InPacket p, Client c) {
        ...
        if (ticket != ItemId.REMOTE_GACHAPON_TICKET) {
            AutobanManager.alert(c.getPlayer(), AutobanFactory.GENERAL, " Tried to use RemoteGachaponHandler with item id: " + ticket);
            c.disconnect(false, false);
            return;
        } else if (gacha < 0 || gacha > 11) {
            AutobanManager.alert(c.getPlayer(), AutobanFactory.GENERAL, " Tried to use RemoteGachaponHandler with mode: " + gacha);

            c.disconnect(false, false);
            return;
        } else if (c.getPlayer().getInventory(ItemConstants.getInventoryType(ticket)).countById(ticket) < 1) {
            AutobanManager.alert(c.getPlayer(), AutobanFactory.GENERAL, " Tried to use RemoteGachaponHandler without a ticket.");
