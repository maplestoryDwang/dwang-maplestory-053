...
function start() {
    ...
    var wearA = cm.getPlayer().isEquipped(1472063); // 是否装备
    ...
    if (cmap == 209000000) { // 幸福村
        if (endTime > 0) {
            ...
            if (val == null || val == "") {
                cm.sendSimple("嘿，那边的人！我是鲁道夫。我在这里做什么？我的工作是确保幸福村永远下雪！但现在我们正面临危机！由于幸福村最近的改建，我们永恒之雪的库存消失了！\r\n\r\n#L0# 什么是永恒之雪？#l\r\n#L1# 不……没什么兴趣。#l");
                status = 1;
            } else if (val == "ing") {
        ...
        cm.sendSimple("你把所有冰冻之雪都交给精灵们了吗？哦，你想回幸福村吗？\r\n#L0# 是的，请带我回去。\r\n#L1# 不，我还有些事要在这做。\r\n#l");
        status = 8;
    } else {
        cm.dispose();
    }
    ...
    if (status == 1 && selection == 0) {
        ...
        if (!wearA) {
            if (itemA == 0) {
                ...
                cm.sendOk("如果你不#r#装备#k#n上#b魔法手套#k，你进入超级冰冻地带时肯定会冻死，相信我。请重新考虑你的决定。");
                cm.dispose();
            }
        } else { // 已装备
            if (itemA > 1) {
